...
Class Cit_Breeds_management extends Breeds_management {
        public function __construct()
{
    parent::__construct();
      $this->load->model('cit_api_model');
}
public function checkUniqueBreed($value = ''){
    $return_arr='1';
    if(false == empty($value)){
      $this->db->select('iBreedsId');
      $this->db->from('user_breed');
      $this->db->where_in('iBreedsId', $value);
      $arrBreedData=$this->db->get()->result_array();
     if(true == empty($arrBreedData)){
         $return_arr = "0";
         return  $return_arr;
      }     
    } 
...
{     
        $url = $this->general->getAdminEncodeURL('basic_appineers_master/breeds_management/add').'|mode|'.$this->general->getAdminEncodeURL('Update').'|id|'.$this->general->getAdminEncodeURL($arr);
