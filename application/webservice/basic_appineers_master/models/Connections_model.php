...
class Connections_model extends CI_Model
{
    ...
    public function connection_details($params_arr = array())
    {
		...
		$result_arr = array();
                ...
                $this->db->where("usc.eConnectionResult", $params_arr['connection_type']);
		...
		// echo $this->db->last_query();exit;
               ...
               // $this->db->select()
                ...
                $this->db->where("usc.eConnectionResult", "Unmatch");
                $this->db->where("usc.iUserId", $params_arr['user_id']);
               ...
               //echo $this->db->last_query();exit;
                ...
                $this->db->where("usc.eStatus", 'Active');
                $this->db->where("u.eStatus", 'Active');
                $this->db->where("usc.eConnectionType",'Like');
                $this->db->where("usc.eConnectionResult", "Unmatch");
                ...
                $this->db->join("users AS u", "u.iUserId = usb.iConnectionUserId", "left");
                $this->db->select("u.iUserId AS user_id");
                $this->db->select("(concat(u.vFirstName,' ',u.vLastName)) AS user_name", FALSE);
                $this->db->select("u.vProfileImage AS user_image");
    ...
    public function get_likeme_count($params_arr = array())
    {
        try {
                ...
                $result_arr = array();
                 ...
                 $this->db->from("user_connections AS usc");            
                $this->db->join("users AS u", "u.iUserId = usc.iUserId", "left");
                $this->db->select("u.iUserId AS user_id2");
                $this->db->select("(concat(u.vFirstName,' ',u.vLastName)) AS user_name2", FALSE);
                $this->db->select("u.vProfileImage AS user_image2");
    ...
    public function get_like_count($params_arr = array())
    {
        try {
                ...
                $this->db->from("user_connections AS usc");            
                $this->db->join("users AS u", "u.iUserId = usc.iConnectionUserId", "left");
                $this->db->select("u.iUserId AS user_id1");
                $this->db->select("(concat(u.vFirstName,' ',u.vLastName)) AS user_name1", FALSE);
    ...
    public function add_connection_status($params_arr = array())
    {
        try
        {
            ...
            if (isset($params_arr["connection_result"]))
            {
                $this->db->set("eConnectionResult", $params_arr["connection_result"]);
            }
 ...
 /**
     ...
     * @return array $return_arr returns response of review block.
     */
    public function update_exist_connection_result_status($params_arr = array(), $where_arr = array())
    {
 ...
 /**
    ...
    public function get_user_details_for_send_notifi($user_id = '', $receiver_id = '',$connection_type='')
    {
        try
        {
            ...
            $this->db->select("r.eIsSubscribed AS r_premium_status");
