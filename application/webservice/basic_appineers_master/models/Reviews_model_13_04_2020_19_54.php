...
class Reviews_model extends CI_Model
{
    ...
    public function get_review_details($arrResult)
    {
        try
        {
            ...
            $this->db->select(" CAST(AVG(r1.iStars) AS Decimal(4, 2))")->join("users AS u", "u.vEmail = r1.vEmail", "inner")->where("r1.eStatus= 'Active' AND r1.bIsClaimed = true AND r1.vClaimedEmail = r.vClaimedEmail")->from("review AS r1"); 
            $subQuery =  $this->db->get_compiled_select();*/

            
            if (isset($arrResult['page_name'] ) && $arrResult['page_name']  != "" && ("consumer_listing" == $arrResult['page_name'] || "review_for_me" == $arrResult['page_name'] || "new_user_listing" == $arrResult['page_name']))
            {
                if (isset($arrResult['email_address']) && false == empty($arrResult['email_address']) && true == empty($arrResult['mobile_number']))
            {
               ...
               $strWhere = "r.vEmail='" . $arrResult['email_address'] . "'";
                ...
                {
                    $strWhere .= " OR r.vFirstName='" . $arrResult['first_name'] . "'";
                }
            ...
            if("new_user_listing" == $arrResult['page_name'])
            {
              $strWhere ="(". $strWhere.")"."  AND FIND_IN_SET('" . $arrResult['email_address'] . "', r.vClaimedEmail) = 0";
            }
            if("review_for_me" == $arrResult['page_name'] )
             {
               $strWhere ="(". $strWhere.")"."  AND FIND_IN_SET('" . $arrResult['email_address'] . "', r.vClaimedEmail)";
