...
class Mad_collab_user extends Cit_Controller
{
    ...
    {
        $valid_arr = array(            
            "media_file_count" => array(
                array(
                    "rule" => "required",
                    "value" => TRUE,
                    "message" => "media_file_count_required",
                )
            ),
            "user_id" => array(
                array(
            ...
            if (is_array($result_arr) && count($result_arr) > 0)
            {
                ...
                foreach ($result_arr as $data_key => $data_arr)
                {
                    ...
                    if(false == empty($arrConnectionType['0']['connection_type'])){
                        ...
                        $result_arr[$data_key]["connection_type"] =  $strConnectionType ;
                    }else{
                        $result_arr[$data_key]["connection_type"] =  $strConnectionType ;
                    }
                    ...
                    $image_arr["path"] = "mad_collab/media_images";
                   ...
                   $data = $this->general->get_image_aws($image_arr);
                    ...
                    $image_arr["color"] = "FFFFFF";
                    $image_arr["no_img"] = FALSE;
                    $dest_path = "mad_collab/user_profile";
                    $image_arr["path"] = $this->general->getImageNestedFolders($dest_path);
                     $data = $this->general->get_image_aws($image_arr);
